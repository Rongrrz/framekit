...
describe('sequences', () => {
  it('creates immutable uniform and explicit sequences', () => {
    ...
    expect(numberSequence(10, 20, 30)).toEqual([
      { Time: 0, Value: 10 },
      { Time: 0.5, Value: 20 },
      { Time: 1, Value: 30 },
    ]);
  ...
  it.each([
    ...
    expect(() => numberSequence(points[0]!, points[1]!, ...points.slice(2))).toThrow(message);
  });

  it('rejects non-finite values and malformed color channels', () => {
    expect(() => numberSequence(0, Number.POSITIVE_INFINITY)).toThrow(/finite/);
    expect(() => colorSequence(color3FromRGB(0, 0, 0), { R: 256, G: 0, B: 0 })).toThrow(/channels/);
  });

  it('rejects mixed shorthand values and explicit keypoints in either order', () => {
    const red = color3FromRGB(255, 0, 0);
    const blue = color3FromRGB(0, 0, 255);
    expect(() => numberSequence(0, { Time: 1, Value: 1 })).toThrow(/cannot mix/);
    expect(() => numberSequence({ Time: 0, Value: 0 }, 1)).toThrow(/cannot mix/);
    expect(() => colorSequence(red, { Time: 1, Value: blue })).toThrow(/cannot mix/);
    expect(() => colorSequence({ Time: 0, Value: red }, blue)).toThrow(/cannot mix/);
  });
});
